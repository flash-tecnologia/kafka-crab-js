  ...
  if (process.env.NAPI_RS_NATIVE_LIBRARY_PATH) {
    ...
    if (process.arch === 'arm64') {
      ...
      try {
        ...
        if (bindingPackageVersion !== '1.10.0' && process.env.NAPI_RS_ENFORCE_VERSION_CHECK && process.env.NAPI_RS_ENFORCE_VERSION_CHECK !== '0') {
...
if (!nativeBinding || process.env.NAPI_RS_FORCE_WASI) {
  try {
    nativeBinding = require('./kafka-crab-js.wasi.cjs')
  } catch (err) {
  ...
  if (!nativeBinding) {
    try {
      nativeBinding = require('kafka-crab-js-wasm32-wasi')
    } catch (err) {
      if (process.env.NAPI_RS_FORCE_WASI) {
        loadErrors.push(err)
      }
